...
from src.helper import color_echo, read_text, read_toml, read_yaml, yaml
from src.helper.constants import RootFile, Template, Workflow
from src.helper.file_handler import append_file, copy_to_local, create_file
...
def _add_hooks_toml(*hooks: str):
    ...
    config = read_text(Template.pre_commit_pref)
    black, flake8, isort = config.split("\n\n")
    config_map = {"black": black, "flake8": flake8, "isort": isort}

    pyproject = RootFile.pyproject_toml.from_path()
...
def add_pre_commit():
    ...
    copy_to_local(source=Workflow.pre_commit, destination=Workflow.pre_commit.to_path())
...
def add_sourcery():
    ...
    copy_to_local(source=RootFile.sourcery_config.template(), destination=RootFile.sourcery_config)
